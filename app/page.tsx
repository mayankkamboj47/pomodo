        ...
        return (<List key={listId} name={list.name} dispatch={dispatch}  id={listId}
                tasks={
                  list.taskOrder.map(taskId=>{
                    ...
                    return (<Task key={taskId} value={task.value} id={taskId}
  ...
  switch (action.type) {
    case 'clock.stop-resume':
      if(state.clockStatus === 'running') {
        ...
        state.clockStatus.match(/^stopped \d+$/)
      ) {
        newState.clockStatus = 'running';
        newState.endTime = Math.floor(new Date().getTime() / 1000) + Number(state.clockStatus.split(' ')[1]);
      }
      else {
        newState.clockStatus = 'running';
      }
    ...
    case 'task.delete':
      ...
      for(let listId in newState.lists) {
        newState.lists[listId].taskOrder = newState.lists[listId].taskOrder.filter((taskId: any) => taskId !== action.taskId);
      }
    break;
    ...
    case 'task.add':
      const newTaskId = Math.max(...Object.keys(newState.tasks).map((taskId: any) => Number(taskId))) + 1;
    ...
    case 'list.add':
      const newListId = Math.max(...Object.keys(newState.lists).map((listId: any) => Number(listId))) + 1;
    ...
    case 'list.delete':
      ...
      newState.listOrder = newState.listOrder.filter((listId: any) => listId !== action.id);
    break;
